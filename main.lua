do
    ...
    subtitleLabel.Text = "RIP BRAINROT"
...
espPlayerToggle.MouseButton1Click:Connect(function()
    ...
    local playerBase = basesFolder:FindFirstChild(tostring(playerBaseIndex))
    if not playerBase then
        Yanz.Console.error("Player base not found!")
        return
    end

    -- ✅ Cek dulu kalau sudah terkunci
    local alreadyLocked = playerBase:FindFirstChild("Gate") ~= nil
    if alreadyLocked then
        return -- jangan spam lock lagi
    end
    ...
    local args = {{
        workspace:WaitForChild("Bases")
                 :WaitForChild(tostring(playerBaseIndex))
    ...
    if not ok then
        Yanz.Console.error("Failed to fire base lock: " .. tostring(err))
    else
